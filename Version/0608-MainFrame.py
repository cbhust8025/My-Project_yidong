...
curFilePath = sys.path[0] if os.path.isdir(sys.path[0]) else  os.path.dirname(sys.path[0]) # 全局变量，当前文件路径
########################################################################
########################################################################
# 盲点检测图片集
MDataPicPath = [
...
class OpenFile(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.contentlist = [x.split(',')[:-1] for x in self.contentSelected]
            if(len(sample) > 5):
                self.contentlist.insert(0, [(u"列" + str(i)) for i in range(len(sample))])
                wx.CallAfter(self.window.DataPanelShow, self.contentlist)
            else:
...
class Demo(wx.Frame):
    def __init__(self):
        ...
        self.choices1 = wx.Choice(
            self.Panel2, -1, choices=self.sampleList1.keys(), style=wx.ALIGN_CENTER)
        ...
        self.sampleListP = {}
        self.choicesP = wx.Choice(
            self.Panel2, -1, choices=self.sampleListP.keys(), style=wx.ALIGN_CENTER)
        ...
        self.datapanel = wx.ListCtrl(self.Panel3, style=wx.LC_REPORT)  # 当前风格支持多列
        self.datapanel.SetFont(wx.Font(18, wx.ROMAN, wx.NORMAL, wx.NORMAL, False))
        # ***************panel 3**********************************************
        ...
        self.Lcbox.Add(self.datapaneltestButton, proportion=0, flag=wx.ALL, border=10)
        ...
        self.P3abox.Add(self.datapanel, proportion=1, flag=wx.EXPAND | wx.ALL, border=0)
        ...
        self.P4abox.Add(self.bar, proportion=1,
                        flag=wx.EXPAND | wx.ALL, border=0)
        self.P4abox.Add(self.box_P41, proportion=1,
                        flag=wx.EXPAND | wx.ALL, border=0)
        ...
        self.wbox.Add(self.tbox, proportion=9,
                      flag=wx.ALL | wx.EXPAND, border=10)
    ...
    def OnEraseBackgroundPane2(self, evt):
        ...
        # path111 = cur_file_dir()
        # print path111
        bmp = wx.Bitmap(curFilePath + "\\" + "bgP2.jpg")
        dc.DrawBitmap(bmp, 0, 0)
    ...
    def CaseThree(self, evt):
        ...
        self.wbox.Layout()
        temp = u'本模块为网络异常分析模块\n\n ---建立异常问题矩阵数据库，针对\
        具体分析导入数据库进行分析，给出相应的问题分析原因及解决方案。'
    ...
    def GridsMsg(self, msg, flag=0):
        ...
        self.grids.AppendText("\n" + cc)
        # self.bar.SetInsertionPoint(0)
        f = wx.Font(15, wx.ROMAN, wx.NORMAL, wx.NORMAL, False)
        self.grids.SetStyle(0, self.bar.GetLastPosition(),
                            wx.TextAttr("black", "white", f))
