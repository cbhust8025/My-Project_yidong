...
class Demo(wx.Frame):
    ...
    def __init__(self):
        ...
        #######################################################################
        # 主窗口的一些设置
        self.bmg1 = wx.Bitmap(curFilePath + "\\" + "bg1.jpg")
        self.bmgP1 = wx.Bitmap(curFilePath + "\\" + "bgP1.jpg")
        ...
        # 问题诊断
        self.ProblemSolName = ['诊断问题', '异常总次数', '异常原因分类', '概率分析', '解决措施']
        ##################################################################

        ###################################################################
        # 异常信令分析 区域选择设置变量
        self.city = u"未选择"  # 当前选择小区
        self.area = u"未选择"  # 当前选择小区
        self.timeslot = u"未选择"  # 当前选择时间段
        ###################################################################
        ...
        CaseOneButton = wx.Button(self.Panel1, label=u'区域盲点检测')
        ...
        self.ProblemOutButton = wx.Button(self.Panel2, label=u'诊断结果导出')
        self.ProblemOutButton.Bind(wx.EVT_BUTTON, self.ProblemOut)
        # ***************panel 2  part 4 --case 3******************************
        # ***************panel 3*****输出信息面板初始化*********************************
        self.grids = wx.TextCtrl(
            self.Panel3, -1, style=wx.TE_MULTILINE | wx.TE_RICH2)
        self.grids.SetEditable(False)
        self.grids.SetFont(
        ...
        self.PPBbox.Add(self.ProblemSolButton, proportion=0,
                        flag=wx.ALL, border=10)
        self.PPBbox.Add(self.ProblemOutButton, proportion=0,
                        flag=wx.ALL, border=10)
    ...
    def DataPanelShow(self, data, width=None):
        ...
        for i, title in enumerate(data[0]):
            ...
            if width is not None:
                self.datapanel.SetColumnWidth(i, width[i])
    ...
    def OnPaint2(self, event):
        ...
        # 防止重复操作 -- 0610:取消绑定会导致切换界面后界面上的背景图片消失没有进行重新绘制
    ...
    def ShowIndex(self):
        fou = open(curFilePath + "\\" + indexfilename.decode("utf8"), 'r+')
    ...
    def CaseShow(self, evt):
        self.ClearList()
        temp = u'本模块为功能模块，共具有三个功能模块：\n\n1、区域盲点检测功能模块 ---主要进行区域盲点检测，关联指标进行筛选盲点用户；\n\n' \
    ...
    def ImportMatrix(self, evt):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.Show_Content(u"成功导入故障矩阵初始文件：" + temp + u"！")
        dlg.Destroy()
    ...
    def ResultOutput(self, evt):
        ...
        if(self.choices6.GetSelection() == 3 and self.choices6_1.GetSelection() == 1):
            ...
            jpg = wx.Image(curFilePath + "\\" + MDataPicPath[self.choices6.GetSelection()][
    ...
    def UserPredict(self, evt):
        ...
        if (self.c4 == -1):
            ...
            # print self.choices4.GetSelection()
            # print self.Panel3.GetSize()
            jpg = wx.Image(curFilePath + "\\" + PDataPicPath[self.c4][self.choices5.GetSelection()],
                           wx.BITMAP_TYPE_JPEG).ConvertToBitmap()
            self.USB = wx.StaticBitmap(self.Panel3, -1, jpg, style=wx.CENTER)
            self.List.append(self.USB)
    ...
    def InitMatrix(self, evt):
        if self.MatrixFile == '':
            ...
            for content in contents:
                temp = content.split(' ')
                if 5 == len(temp):
                    if not temp[0] in self.sampleListP:
    ...
    def ShowMatrix(self):
        ...
        # 利用self.DataPanelShow来进行显示
        # 数据显示面板进行显示送进来的数据，以列表形式，第一行为表头，其他的每一行分别为数据元素值
        showdata = [[k.decode('utf8') for k in self.MatrixName]]
        if 0 == len(self.Matrix[0]):
            return
        for i in range(len(self.Matrix[0])):
            showdata.append([k[i].decode('gbk') for k in self.Matrix])
        self.DataPanelShow(showdata, [160, 200, 125, 205, 200])
        # 利用Set_grids来进行显示
        # temp = u"当前矩阵表简况:" + "\n"
        #
    ...
    def ProblemOut(self, evt):
        # 异常信令分析的问题诊断结果到处按钮对应事件函数
        dlg = wx.FileDialog(
        ...
        if dlg.ShowModal() == wx.ID_OK:
            savefilename = dlg.GetPaths()[0]
            # print savefilename
            fout = open(savefilename, 'w+')
            # print self.datapanel.GetItemCount(), " ", self.datapanel.GetColumnCount()
            title = [fout.write(k.decode('utf8').encode('gbk') + ",") for k in self.ProblemSolName]
            fout.write("\n")
            for i in range(self.datapanel.GetItemCount()):
                # print self.datapanel.GetItemText(i)
                for j in range(self.datapanel.GetColumnCount()):
                    print "col: " + str(j) + "  row: " + str(i) + "  " + self.datapanel.GetItemText(i, col=j)
                    fout.write(self.datapanel.GetItemText(i, col=j).rstrip("\n").encode('gbk') + ",")
                fout.write("\n")
            fout.close()
...
class HAddMatrixDialog(wx.Dialog):  # 手动扩充矩阵表对话框
    ...
    def __init__(self, parent):
        super(HAddMatrixDialog, self).__init__(
            parent, title=u"矩阵表-手动扩充对话框", size=(700, 400))
        self.parent = parent
        f = wx.Font(14, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL,
                    wx.FONTWEIGHT_NORMAL, False, face=u"华文新宋")
        ...
        self.P2TC = wx.TextCtrl(
            self.Panel_ds, -1, u'因为PUCCH资源分配失败而导致RRC连接建立失败的次数')
        self.P3 = wx.StaticText(
            self.Panel_ds, -1, u"故障关联指标项计数:", style=wx.ALIGN_CENTER)
        self.P3TC = wx.TextCtrl(self.Panel_ds, -1, r'4')
        self.P4 = wx.StaticText(
            self.Panel_ds, -1, u"诊断问题:", style=wx.ALIGN_CENTER)
        self.P4TC = wx.TextCtrl(self.Panel_ds, -1, u'下行业务受弱覆盖影响较大导致PUCCH资源分配失败')
        ...
        self.P221box.Add(self.ProblemT, proportion=0, flag=wx.ALL | wx.EXPAND, border=0)
        self.P222box = wx.BoxSizer()
        ...
        sb1 = wx.StaticBox(self.Panel_ds, label=u"矩阵表-手动扩充:")  # 静态box布局
        self.P22box = wx.StaticBoxSizer(sb1, wx.VERTICAL)
        ...
        self.P3box = wx.BoxSizer(wx.VERTICAL)
        self.P3box.Add(self.Panel_ds, proportion=0,
                       flag=wx.ALL | wx.EXPAND, border=10)
        self.P3box.Add(self.Panel_ds2, proportion=0, flag=wx.ALL, border=10)
    ...
    def Set(self, evt):
        try:
            ...
            if (not self.parent.sampleListP.has_key(pro)):
                ...
                self.parent.sampleListP[pro] += 1
            self.parent.ShowMatrix()
            self.Show(False)
        except:
            self.parent.Show_Content(u"手动添加失败，未初始化矩阵表！")
            self.Show(False)
...
class ProblemSolDialog(wx.Dialog):
    # 问题诊断设置对话框

    def __init__(self, parent):
        super(ProblemSolDialog, self).__init__(
            parent, title=u"异常信令事件分析-问题诊断", size=(700, 300))
        self.parent = parent
        f = wx.Font(14, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL,
                    wx.FONTWEIGHT_NORMAL, False, face=u"华文新宋")
        self.SetFont(f)
        ...
        sb1 = wx.StaticBox(self, label=u"问题诊断设置:")  # 静态box布局
        self.P22box = wx.StaticBoxSizer(sb1, wx.VERTICAL)
    ...
    def Set(self, evt):
        ...
        # print total, 3
        print total
        print self.parent.problem
        # 利用DataPanelShow来进行显示问题诊断结果
        showdata = [[k.decode('utf8') for k in self.parent.ProblemSolName]]
        print showdata
        for i in range(len(self.parent.Matrix[0])):
            if self.parent.Matrix[3][i].decode('gbk') in mmm:
                temp = [self.parent.problem, str(total), self.parent.Matrix[3][i].decode('gbk'),
                        str(100 *
                            float(self.parent.Matrix[2][i]) / float(total)),
                        self.parent.Matrix[4][i].decode('gbk')]
                showdata.append(temp)
        self.parent.DataPanelShow(showdata, [160, 140, 185, 125, 280])
        # 利用Set_grids来进行显示
        ...
        self.Show(False)
        self.parent.Show_Content(u"问题诊断：" + self.parent.problem)
...
class ResionSelectDialog(wx.Dialog):
    # 区域选择对话框

    def __init__(self, parent):
        super(ResionSelectDialog, self).__init__(
            parent, title=u"异常信令事件分析-区域选择", size=(500, 300))
        self.Center()
        self.parent = parent
        f = wx.Font(14, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL,
                    wx.FONTWEIGHT_NORMAL, False, face=u"华文新宋")
        self.SetFont(f)
        self.fgs = wx.FlexGridSizer(2, 1)  # 2行1列自动调整二维布局
        sb1 = wx.StaticBox(self, label=u"异常信令时间分析--设置")  # 静态box布局
        self.sbs = wx.StaticBoxSizer(sb1, wx.VERTICAL)
        s1 = wx.StaticText(self, id=-1, label=u"区域选择：")
        s2 = wx.StaticText(self, id=-1, label=u"小区选择：")
        s3 = wx.StaticText(self, id=-1, label=u"时段选择：")
        self.c1 = wx.Choice(self, id=-1, choices=[u"武汉"])
        self.c2 = wx.Choice(self, id=-1, choices=[u"光谷金融港ZLH-1"])
        self.c3 = wx.Choice(self, id=-1, choices=[u"2016年5月7日14:00-16:00"])
        b1 = wx.Button(self, id=-1, label=u"确定")
        b2 = wx.Button(self, id=-1, label=u"取消")

        b1.Bind(wx.EVT_BUTTON, self.Set)
        b2.Bind(wx.EVT_BUTTON, self.exit_out)
        box1 = wx.BoxSizer()
        box1.Add(b1, flag=wx.ALL, border=10)
        box1.Add(b2, flag=wx.ALL, border=10)
        self.sbs.AddMany([s1, self.c1,
                          s2, self.c2,
                          s3, self.c3])
        self.fgs.Add(self.sbs, flag=wx.LEFT | wx.RIGHT, border=40)
        self.fgs.Add(box1, flag=wx.LEFT | wx.TOP, border=20)
        self.SetSizer(self.fgs)

    def Set(self, evt):
        # 设置需要分析的原因,并根据选择来进行计算诊断结果
        # print self.c1.GetStringSelection()
        self.Parent.city = self.c1.GetStringSelection()
        self.Parent.area = self.c2.GetStringSelection()
        self.Parent.timeslot = self.c3.GetStringSelection()
        text = u"当前选择区域： " + self.Parent.city
        text += u"    当前小区： " + self.Parent.area + "\n"
        text += u"当前选择时间段： " + self.Parent.timeslot + "\n"
        self.Parent.Show_Content(text)
        self.Show(False)
